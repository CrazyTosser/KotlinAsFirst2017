...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    val resCount = MutableList(substrings.size) { 0 }
    (File(inputName).readLines()).forEach { line ->
        var i = 0
        while (i < substrings.size) {
            resCount[i] += line.toLowerCase().count(substrings[i].toLowerCase())
            i++
        }
    }
    (0 until substrings.size).forEach {
        res.put(substrings[it], resCount[it])
    }
    return res
}

private fun String.count(predicate: String): Int =
        when {
            this.isEmpty() -> 0
            else -> (0..this.length - predicate.length).count { this.substring(it, it + predicate.length) == predicate }
        }
...
fun sibilants(inputName: String, outputName: String) {
    ...
    for ((index, line) in inp.withIndex()) {
        ...
        (line).forEach { c ->
            if (type) {
                when (c) {
                    'ы', 'Ы' -> if (c == 'ы') outputStream.write("и")
                    else outputStream.write("И")
                    'я', 'Я' -> if (c == 'я') outputStream.write("а")
                    else outputStream.write("А")
                    'ю', 'Ю' -> if (c == 'ю') outputStream.write("у")
                    else outputStream.write("У")
                    else -> outputStream.write(c.toString())
                }
                type = false
            } else {
